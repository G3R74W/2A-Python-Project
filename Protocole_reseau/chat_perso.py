...
class Communicator():
  ...
  def startCommunicator(self):
    if self.connexion:
      ...
      listenThread = threading.Thread(target=Communicator.listenLoop,args=(self,self.reactionFonction,self.reactionFonctionObject,),daemon=True)
      ...
      messageThread = threading.Thread(target=Communicator.messageLoop,args=(self,self.entreeFonction,self.entreeFonctionObject,),daemon=True)
...
##    if data:
##      # on encode pour passer d'une string a des donnes brutes
##      print("| %20s    sending:(%s)" %(threading.current_thread().name,repr(data)))
##      # envoi via la <SOCKET>
